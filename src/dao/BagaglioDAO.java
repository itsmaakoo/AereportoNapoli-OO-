...
public class BagaglioDAO {
    ...
    public List<Bagaglio> trovaBagagliSmarriti() throws SQLException {
        ...
        try (PreparedStatement stmt = connessione.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                try {
                    b.setStato(StatoBagaglio.valueOf(statoString));
                } catch (IllegalArgumentException e) {
                    b.setStato(StatoBagaglio.valueOf(statoString.toLowerCase()));
                }
                bagagli.add(b);
            }
        }
     ...
     *
     * @param codiceBagaglio codice del bagaglio (passato come String ma convertito in int)
    ...
    public void aggiornaStatoBagaglio(String codiceBagaglio, StatoBagaglio nuovoStato) throws SQLException {
        ...
        try (PreparedStatement stmt = connessione.prepareStatement(query)) {
            ...
            stmt.setInt(2, Integer.parseInt(codiceBagaglio));
            stmt.executeUpdate();
        } catch (NumberFormatException e) {
            throw new SQLException("Il codice del bagaglio fornito non è un numero valido: " + codiceBagaglio);
        }
    }
}
